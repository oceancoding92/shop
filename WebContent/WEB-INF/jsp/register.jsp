...
//表单利用js进行验证是否为空，简单的逻辑判断
function checkForm(){
	var username=document.getElementById("username").value;
	if(username==null||username==""){
		alert("用户名不为空");
		/* document.getElementById("username_blank").innerHTML; */
		return false;
	}
...
function checkUsername(){
	...
	xmlHttp.open("GET","${pageContext.request.contextPath}/user_findByusername.action?time="+new Date().getTime()+"&username="+username,true);	
	//发送
	xmlHttp.send(null);
	}
...
function change(){
	var image1=document.getElementById("checkImg");
	image1.src="${pageContext.request.contextPath}/checkImg.action?"+new Date().getTime();
}
</script>
</head>
<body>
						...
						<table>
							...
							<tr>
								...
								<tr>
									...
									<td>
										<span class="fieldSet">
											<input type="text" id="checkcode" name="checkcode" class="text captcha" maxlength="4" autocomplete="off"><img id="checkImg" class="captchaImage" src="${pageContext.request.contextPath}/checkImg.action" onclick="change()" title="点击更换验证码"><div ><font color="red"><s:actionerror /></font></div>
										</span>
